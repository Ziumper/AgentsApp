...
namespace Rae {
	...
	std::vector<Cycle> MonteCarlo::CreateCycles()
	{
	...
	void MonteCarlo::Start()
	{
		...
		std::vector<Cycle> cycles = this->CreateCycles();
		
		//preserve agents to next cycle
		std::vector<Agent> tempAgents = this->CreateAgents();
